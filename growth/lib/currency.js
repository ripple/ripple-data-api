...
ripple.currencyDropdown = function() {
  ...
  function loaded(selection) {
    var currencies = ["XRP"].concat(d3.keys(accountsByCurrency).sort()),
        ...
        gateway = selected && gatewayByAddress[selected[1]];
    ...
    var option = currencySelect.selectAll("option")
      ...
      .enter().append("option")
        .text(String);
    if (selected) option.property("selected", function(d) { return d === selected[0]; });
    change();
    ...
    function change() {
      ...
      event.change(address ? [currency, address] : [currency]);
    }
  }
